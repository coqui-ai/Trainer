...
class TrainerArgs(Coqpit):
    ...
    grad_accum_steps: int = field(default=1, metadata={"help": "Number of gradient accumulation steps. It is used to accumulate gradients over multiple batches."})
    overfit_batch: bool = field(default=False, metadata={"help": "Overfit a single batch for debugging."})  # TODO: implement
    skip_train_epoch: bool = field(default=False, metadata={"help": "Skip training and only run evaluation and test for debugging."})
...
class Trainer:
        ...
        if self.num_gpus > 1:
            if hasattr(self.model.module, "get_train_data_loader"):
                loader = self.model.module.get_train_data_loader(
                    self.config, self.assets, data_items, verbose, self.num_gpus, self.args.rank
                )
                return loader
        else:
            if hasattr(self.model, "get_train_data_loader"):
                loader = self.model.get_train_data_loader(self.config, self.assets, data_items, verbose, self.num_gpus)
                return loader
    ...
    def get_eval_dataloader(self, training_assets: Dict, data_items: List, verbose: bool) -> DataLoader:
        ...
        if self.num_gpus > 1:
            ...
            if hasattr(self.model, "get_eval_data_loader"):
                loader = self.model.get_eval_data_loader(self.config, self.assets, data_items, verbose, self.num_gpus)
                return loader
    ...
    def get_test_dataloader(self, training_assets: Dict, data_items: List, verbose: bool) -> DataLoader:
        ...
        if self.num_gpus > 1:
            ...
            if hasattr(self.model, "get_test_data_loader"):
                loader = self.model.get_test_data_loader(self.config, self.assets, data_items, verbose, self.num_gpus)
                return loader
    ...
    def test_run(self) -> None:
        ...
        if hasattr(self.model, "test_run") or (self.num_gpus > 1 and hasattr(self.model.module, "test_run")):
            ...
            if hasattr(self.test_loader.dataset, "load_test_samples"):
                ...
                if self.num_gpus > 1:
                    self.model.module.test_run(self.training_assets, self.test_loader, None)
                else:
    ...
    def test(self, model=None, test_samples=None) -> None:
        ...
        self.c_logger.print_epoch_end(
            self.epochs_done, self.keep_avg_eval.avg_values
        )
